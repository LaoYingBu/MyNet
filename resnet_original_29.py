...
def residual_block(bottom, num_filters, stride=1):
    if stride == 1:
        ...
        conv2 = conv_factory_relu(conv1, 3, num_filters, 1, 1)
        conv3 = conv_factory(conv2, 1, num_filters * 4, 1, 0)
        add = L.Eltwise(bottom, conv3, operation=P.Eltwise.SUM)
        return add
    elif stride == 2:
        conv1 = conv_factory_relu(bottom, 1, num_filters, 1, 0)
        conv2 = conv_factory_relu(conv1, 3, num_filters, 2, 1)
        conv3 = conv_factory(conv2, 1, num_filters * 4, 1, 0)
...
if __name__ == '__main__':
    tgt_file = 'D:/MyNet/livdet2015/resnet_29_train_test.prototxt'
    make_net(tgt_file)
