...
def residual_block(bottom, num_filters, stride=1):
    if stride == 1:
        ...
        drop1 = L.Dropout(conv1, dropout_ratio=0.5, in_place=True)
        conv2 = conv_factory(drop1, 3, num_filters, 1, 1)
...
def resnet_cifar(train_lmdb, test_lmdb, mean_file, batch_size=100, n_size=3):
    ...
    for i in xrange(n_size - 1):
        residual = residual_block(residual, 16 * 2)

    # --------------> 32, 16, 16    2nd group
    residual = residual_block(residual, 32 * 2, 2)
    for i in xrange(n_size):
        residual = residual_block(residual, 32 * 2)

    # --------------> 64, 8, 8      3rd group
    residual = residual_block(residual, 64 * 2, 2)
    for i in xrange(n_size):
...
if __name__ == '__main__':
    tgt_file = 'D:/MyNet/livdet2015/WRN_16_train_test_compress.prototxt'
    make_net(tgt_file)
