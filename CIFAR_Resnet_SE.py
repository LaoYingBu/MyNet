...
def se_module(bottom, num_filters):
    global_pool = L.Pooling(bottom, pool=P.Pooling.AVE, global_pooling=True)
    fc_8 = L.InnerProduct(global_pool, param=[dict(lr_mult=1, decay_mult=1), 
                                              dict(lr_mult=2, decay_mult=1)],
                          num_output=num_filters/8,
                          bias_term=True,
                          bias_filler=dict(type='constant', value=0), 
                          weight_filler=dict(type='gaussian', std=0.01))
    relu = L.ReLU(fc_8, in_place=True)
    fc = L.InnerProduct(relu, param=[dict(lr_mult=1, decay_mult=1), 
                                     dict(lr_mult=2, decay_mult=1)],
                        num_output=num_filters,
                        bias_term=True,
                        bias_filler=dict(type='constant', value=0),
                        weight_filler=dict(type='gaussian', std=0.01))
    top = L.Sigmoid(fc, in_place=True)
    return top
...
def residual_block(bottom, num_filters, stride=1):
    if stride == 1:
        ...
        se = se_module(conv2, num_filters)
        scale = L.Scale(conv2, se, in_place=False, axis=0, bias_term=False)
        add = L.Eltwise(bottom, scale, operation=P.Eltwise.SUM)
        return add
    elif stride == 2:
        ...
        add = L.Eltwise(pad, scale, operation=P.Eltwise.SUM)
        return add
    else:
...
# ==========================Note here==============================
# !!! SO YOU have to include TRAIN && TEST by your own 
# AFTER you use the script to generate the prototxt !!!
...
if __name__ == '__main__':
    tgt_file = 'D:/MyNet/livdet2015/resnet_20_train_test_SE.prototxt'
    make_net(tgt_file)
